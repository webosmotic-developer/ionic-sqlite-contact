...
export class AddContactPage {
  ...
  constructor(public navCtrl: NavController,
    ...
    this.paramData ? this.contactObj = this.paramData : this.contactObj = {};
    this.listPage = ContactListPage;
    ...
    this.imagePicker.getPictures(options).then((results) => {
      for (var i = 0; i < results.length; i++) {
          ...
          this.base64.encodeFile(results[i]).then((base64File: string) => {
            this.contactObj.avatar = base64File;
          }, (err) => {
            console.log('Error in converting image to base64', err);
          });
      }
    }, (err) => {
      console.log('Error occurred while getting pictures', err);
    });
  }
  ...
  getLoginUser() {
    this._user.getStorage('userInfo').then(res => {
      ...
      this.contactObj.username = this.loggedInUser.username;
    }).catch(err => {
  ...
  addContact() {
    this._contact.addContact(this.contactObj).then(res => {
      this.navCtrl.push(this.listPage);
    }).catch(err => {
      console.log('problem in adding Contact', err);
    });
  }

  updateContact() {
    this._contact.updateContact(this.contactObj).then(res => {
      this.navCtrl.push(this.listPage);
    }).catch(err => {
      console.log('problem in updating Contact', err);
    });
  }
}
